  ...
  async finishCreateClass() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMsg = err.error || 'Failed to save class';
        console.error('Create Class API Error:', err, 'Status:', response.status);
        utils.showStatus('Error: ' + errorMsg, true);
      }

    } catch (e) {
      console.error('Create Class Exception:', e);
      utils.showStatus('Network error occurred. Check console for details.', true);
    }
  },
...
const profileManager = {
  ...
  async saveProfile() {
    ...
    try {
      const response = await fetch(window.djangoUrls.editProfile, {
        method: 'POST',
        headers: {
          'X-CSRFToken': getCsrfToken() // <--- FIXED: Uses the robust helper function
        },
        body: formData
      });
  ...
  // Add these functions to your existing profileManager object

  initiateResign() {
    document.getElementById('modal-resign-otp').classList.remove('hidden');
    document.getElementById('modal-resign-otp').classList.add('flex');
    // Reset state
    document.getElementById('resign-step-1').classList.remove('hidden');
    document.getElementById('resign-step-2').classList.add('hidden');
    document.getElementById('resign-otp-input').value = '';
  },

  async sendResignOtp() {
    const btn = document.querySelector('#resign-step-1 button');
    btn.disabled = true; btn.textContent = "Sending...";

    try {
      const res = await fetch('/api/send-resign-otp/', {
        method: 'POST',
        headers: { 'X-CSRFToken': getCsrfToken() }
      });

      if (res.ok) {
        document.getElementById('resign-step-1').classList.add('hidden');
        document.getElementById('resign-step-2').classList.remove('hidden');
        utils.showStatus("OTP Sent!");
      } else {
        const d = await res.json();
        alert(d.error || "Error sending OTP");
      }
    } catch (e) { console.error(e); alert("Network Error"); }
    finally { btn.disabled = false; btn.textContent = "Send OTP to Email"; }
  },

  async confirmResign() {
    const otp = document.getElementById('resign-otp-input').value;
    if (!otp) return alert("Enter OTP");

    try {
      const res = await fetch('/api/leave-college/', {
        method: 'POST',
      ...
      if (res.ok) {
        alert(data.message);
        window.location.reload(); // Refresh to update UI (remove college name)
      } else {
        alert(data.error);
      }
    } catch (e) { console.error(e); alert("Network Error"); }
...
const notifMgr = {
  toggle() {
    ...
    if (dd.classList.contains('hidden')) {
      ...
      this.fetchAll(); // Changed to fetchAll
    } else {
  ...
  async fetchAll() {
    try {
      // 1. Fetch Class/Join Requests (Existing)
      const reqRes = await fetch(window.djangoUrls.getRequests);
      const reqData = await reqRes.json();
      const requests = reqData.requests || [];

      // 2. Fetch System Notifications (Student Deletions, etc.)
      // Ensure window.djangoUrls.getNotifications points to 'get_faculty_notifications_api' in views
      const notifRes = await fetch(window.djangoUrls.getNotifications);
      const notifData = await notifRes.json();
      const notifications = notifData.notifications || [];

      // 3. Merge and Render
      this.render(requests, notifications);

    } catch (e) { console.error("Notif Error", e); }
  },

  render(requests, notifications) {
    ...
    if (totalCount > 0) {
      ...
      // --- B. RENDER NOTIFICATIONS (Passive Alerts - Student Deleted) ---
      notifications.forEach(notif => {
        const item = document.createElement('div');
        item.className = "p-3 hover:bg-slate-700/50 rounded-lg transition border-b border-slate-700/50 last:border-0 group relative";

        // This handles the "Mark as Read" action
        item.innerHTML = `
            <div class="flex items-start gap-3">
                <div class="p-2 bg-red-900/30 rounded-full text-red-400 mt-1"><i data-lucide="bell" class="w-4 h-4"></i></div>
                <div class="flex-1">
                    <p class="text-sm text-slate-200 leading-snug">${notif.message}</p>
                    <p class="text-[10px] text-slate-500 mt-1">${notif.date}</p>
                </div>
                <button onclick="notifMgr.markRead(${notif.id}, this)" class="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-white p-1 transition" title="Mark as Read">
      ...
      list.innerHTML = `<p class="text-xs text-slate-500 text-center py-4">No new notifications</p>`;
    }
  },

  async markRead(id, btnElement) {
    // 1. Remove UI Element immediately
    const row = btnElement.closest('div.p-3'); // Select the wrapper div
    if (row) {
      row.style.opacity = '0';
    ...
    // 3. Call Backend
    try {
      await fetch(window.djangoUrls.markNotifRead, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': getCsrfToken() },
        body: JSON.stringify({ id: id })
      });

      // OPTIONAL: Reload classes to reflect changes (e.g., if a student was deleted)
...
document.addEventListener('DOMContentLoaded', () => {
  if (window.notifMgr) {
    notifMgr.fetchAll(); // <--- UPDATED: Calls the combined fetcher
  }
});
